-- Fix transfer_dr to handle users without Telegram usernames
-- Previously: LOWER(username) = LOWER(input) fails when username is NULL
-- Now: Try username first, fall back to first_name match
...
BEGIN
  ...
  -- If no match by username, try matching by first_name (case-insensitive)
  IF NOT FOUND THEN
    SELECT * INTO v_recipient FROM public.users
    WHERE LOWER(first_name) = LOWER(p_recipient_username);

    -- If multiple users share the same first_name, reject to avoid ambiguity
    IF FOUND THEN
      DECLARE
        v_count INTEGER;
      BEGIN
