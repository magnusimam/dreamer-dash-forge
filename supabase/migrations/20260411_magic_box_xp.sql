-- Update claim_magic_box to also award bonus XP
...
BEGIN
  ...
  -- Award XP prize
  IF v_box.prize_xp > 0 THEN
    UPDATE public.users SET bonus_xp = bonus_xp + v_box.prize_xp WHERE id = p_user_id;
    INSERT INTO public.transactions (user_id, type, amount, description, reference_id)
    VALUES (p_user_id, 'bonus', 0, '+' || v_box.prize_xp || ' XP from Magic Box: ' || v_box.title, v_box.id);
  END IF;
...
-- Update community stats to include bonus_xp in engagement
...
BEGIN
  ...
  SELECT
    ...
    ) + COALESCE(u.bonus_xp, 0))::BIGINT AS engagement
  FROM public.users u
...
-- Update claim_level_reward to include bonus_xp
...
BEGIN
  ...
  ) + COALESCE(v_user.bonus_xp, 0))::INTEGER INTO v_engagement;
